  ...
  try {
    ...
    if (agentIds.length > 0) {
      const agents = await db
        ...
        .from(aiAgents)
        .where(and(inArray(aiAgents.id, agentIds), eq(aiAgents.userId, session.user.id)))
    ...
    const [conversation] = await db.transaction(async (tx) => {
      ...
      const [newConversation] = await tx.insert(conversations).values(validatedData).returning()
