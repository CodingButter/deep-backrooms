...
export const insertUserSchema = createInsertSchema(users, {
  ...
  email: z.string().email("Invalid email address").max(255, "Email must be less than 255 characters").toLowerCase(),
...
export const insertAiAgentSchema = createInsertSchema(aiAgents, {
  ...
  model: z.string().min(1, "Model is required").max(50, "Model name must be less than 50 characters"),
...
export const insertProviderSchema = createInsertSchema(providers, {
  name: z.string().min(1, "Provider name is required")
    ...
    .regex(/^[a-zA-Z0-9 _-]+$/, "Provider name can only contain alphanumeric characters, spaces, underscores, and hyphens"),
  ...
  defaults: z.string().optional().transform((val, ctx) => {
    if (!val) return null;
    try {
      const parsed = JSON.parse(val);
  ...
  rateLimits: z.string().optional().transform((val, ctx) => {
    if (!val) return null;
    try {
      const parsed = JSON.parse(val);
      if (typeof parsed !== 'object' || parsed === null) {
  ...
  createdAt: z.number().optional().default(() => new Date().getTime()),
